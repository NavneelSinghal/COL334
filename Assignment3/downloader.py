...
def downloadSingleThread(threadNumber, serverName, pipelineSize, fileName):
...
def getContentLength(clientSocket):
    global fileSize
    headerSoFar = ''
    while True:
        receivedMessage = clientSocket.recv(1024)
        if receivedMessage == b'':
            raise RuntimeError('Broken connection')
        headerSoFar += (receivedMessage).decode('ASCII')
        if headerSoFar.lower().find('404 not found') != -1:
            fileSize = -1
            return
        pos = headerSoFar.find('\r\n\r\n')
        if pos != -1:
            headerSoFar = headerSoFar[:pos + 4]
            lengthKeyPosition = headerSoFar.find('Content-Length: ')
            assert lengthKeyPosition != -1
            startValuePosition = lengthKeyPosition + len('Content-Length: ')
            endValuePosition = startValuePosition
            while headerSoFar[endValuePosition] != '\r':
                endValuePosition += 1
            fileSize = int(headerSoFar[startValuePosition : endValuePosition])
            return
    fileSize = -1
    return
...
def downloadMultipleConnections(l):
    global totalChunks, completeChunks, d, fileSize, chunkSize
    assert len(l) >= 1
    # find fileSize by sending request to the first server in l
    for ((serverName, filePath), _) in l:
        # we can test for whether the resource is at these servers by sending a head request
        # but we will assume that it exists, raising an exception if it is not at the first one
        serverPort = 80
        message = "HEAD " + filePath + " HTTP/1.1\r\nHost: " + serverName + "\r\nConnection: keep-alive\r\n\r\n"
        clientSocket = None
        while True:
            try:
                getContentLength(clientSocket)
                break
            except:
                try:
                    clientSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ...
        break
    if fileSize == -1:
        raise RuntimeError('File not found at the server', serverName)
    ...
    t = []
    threadCount = 0
    for ((url, filePath), numThreads) in l:
        t += [threading.Thread(target=downloadSingleThread, args=(threadCount + i, url, pipelineSize, filePath)) for i in range(numThreads)]
...
def main():
    ...
    #downloadChunkWise()
    #downloadPipelined()
    #l = [(('vayu.iitd.ac.in', '/big.txt'), 10)]
    #l = [(('norvig.com', '/big.txt'), 10)]
    assert len(sys.argv) > 1
    with open(sys.argv[1], newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        l = [(parseUrl(line[0]), int(line[1])) for line in list(reader)]
